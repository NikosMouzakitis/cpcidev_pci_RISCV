...
#define EDU_DEVICE_ID 0xabcd //use same as example.
...
static long dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if(cmd == 4) 
	{
		...
		printk(KERN_INFO "retrieve op1 from %x.\n",(void*)(mmio + 0x10));
		ip =  (void *) arg;	
		int vr;	

		//use ioread32()	
		vr = ioread32((void*)(mmio + 0x10));
		printk(KERN_INFO "op1: %x\n",vr);

		// Use copy_to_user to pass the value to user space
		if (copy_to_user(ip, &vr, sizeof(int))) {
			printk(KERN_ERR "Failed to copy data to user space.\n");
			return -EFAULT;
		}
		printk(KERN_INFO "Value successfully copied to user space.\n");
	}

	if(cmd == 5) 
	{
		printk(KERN_INFO "retrieve op2 from %x.\n",(void*)(mmio + 0x14));
		ip =  (void *) arg;	
		int vr;	

		//use ioread32()	
		vr = ioread32((void*)(mmio + 0x14));
	...
	if(cmd == 6) 
	{
		...
		printk(KERN_INFO "retrieve opcode from %x.\n",(void*)(mmio + 0x18));
		ip =  (void *) arg;	
		int vr;	

		//use ioread32()	
		vr = ioread32((void*)(mmio + 0x18));
...
static ssize_t read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	ssize_t ret;
	u32 kbuf;
        printk(KERN_INFO "CPCIDEV: read() len: %lx offset: %llx\n",len, *off);
	if (*off % 4 || len == 0) {
		ret = 0;
	} else {
...
{
	printk(KERN_INFO "CPCIDEV %s\n",__func__);
	u8 val;
	unsigned i;

	pdev = dev;
	/* 1Mb, as defined by the "1 << 20" in QEMU's memory_region_init_io. Same as pci_resource_len. */
	printk(KERN_INFO "CPCIDEV %s call to pci_resource_start \n",__func__);
	resource_size_t start = pci_resource_start(pdev, BAR);
	printk(KERN_INFO "CPCIDEV %s call to pci_resource_end \n",__func__);
	...
	printk(KERN_INFO "CPCIDEV %s call to iomap \n",__func__);
	mmio = pci_iomap(pdev, BAR, pci_resource_len(pdev, BAR));
	printk(KERN_INFO "CPCIDEV mmio: %x \n",mmio);
	...
	printk(KERN_ALERT "pci_probe() returns\n");
	return 0;
error:
	return 1;
}
...
{
	printk(KERN_INFO "CPCIDEV my init on custom_qemu_device_driver\n");
